...
if [ "$mybatis_repo" == "https://github.com/mybatis/freemarker-scripting.git" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_BRANCH" == "master" ] && [[ "$commit_message" != *"[maven-release-plugin]"* ]]; then
  if [ $VER == "17" ]; then
